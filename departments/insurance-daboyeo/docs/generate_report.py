...
def page6_tech(c):
    ...
    status = [
        '[완료]  고객용 앱 MVP (설계사 URL 파라미터 인식)',
        '[완료]  설계사용 앱 MVP (프로필 설정 · 링크 생성 · QR)',
        '[완료]  청구 서류 자동 안내 (8종 케이스)',
        '[완료]  Vercel API 서버 구축 (텔레그램 프록시)',
        '[완료]  경쟁사 차별화 포지셔닝 (전 보험사 실시간 연동)',
        '[완료]  자매 서비스 CODEF API 정식 계약 신청 (세금N혜택)',
        '[진행]  경쟁사 수집부 가동 (보맵 · 굿리치 · 레몬클립 일별 모니터링)',
        '[대기]  AZ금융 승인 및 전사 배포 검토',
        '[대기]  내보험다보여 정식 API 연동 (승인 후)',
