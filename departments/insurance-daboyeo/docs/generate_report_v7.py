#!/usr/bin/env python3
"""
AZ금융 제안서 v7 (보맵 약점 보완 중심 · AZ 사내 ROI 강조)
경영진 피드백 반영:
...
def p1(c):
    ...
    c.drawCentredString(W/2, fy - 2*mm, f'제안일 : {datetime.datetime.now().strftime("%Y년 %m월 %d일")}  (v7)')
    c.showPage()
...
def p6(c):
    ...
    y = bullets(c, y, [
        ...
        '장기적 : 그룹 자산화 시 AZ-보맵 시너지 수치화 가능 (IR·투자유치 재료)',
    ])
...
def p7(c):
    ...
    phases = [
        ...
        ('Phase 4  (3개월~)', '그룹 자산화', RED, [
            '보맵 B2C + 보험다보여 B2B 공식 상호 연계',
            '수익 공유 구조 검토 (AZ 보유 지분 활용)',
            '외부 GA/법인 판매 옵션 (월 29,000원 SaaS)',
            'IR 재료화 : AZ-보맵 그룹 시너지 수치',
        ]),
    ]
...
def main():
    out = Path(__file__).parent / 'AZ금융_제안서_v7_보맵보완중심.pdf'
