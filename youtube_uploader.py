...
import os, json, re, random, math, time, socket, ssl, http.client
...
def upload_to_youtube(video_path, title, description, tags, category="10", privacy="public", playlist_name=""):
    ...
    대용량 파일에서 자주 발생하는 SSL EOF / 네트워크 끊김에 대응하기 위해
    next_chunk() 단위로 재시도(이어올리기)한다. 외부에서 함수 전체를 재시도하면
    1.7GB 같은 큰 파일을 매번 처음부터 올려야 해서 사실상 성공하지 못한다.
    """
    try:
    ...
    # 큰 파일 업로드 중 소켓이 죽는 것을 줄이기 위해 기본 타임아웃 상향
    try:
